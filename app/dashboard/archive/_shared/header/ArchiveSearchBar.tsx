...
import { ARCHIVE_QUERY_NAME, ARCHIVE_QUERY_PARISH } from "../hooks/useArchiveFilters";
...
function ArchiveSearchBar() {
  ...
  const filteredParishes = parishSearch
    ? parishes.filter((p) =>
        p.parish_name.toLowerCase().includes(parishSearch.toLowerCase()),
      )
    : parishes;
  ...
  return (
    <div className="flex w-full items-center justify-between gap-5">
      ...
      <Select value={parishValue} onValueChange={onParishChange}>
        ...
        <SelectContent position="popper" sideOffset={8} className="p-3 w-(--radix-select-trigger-width)">
